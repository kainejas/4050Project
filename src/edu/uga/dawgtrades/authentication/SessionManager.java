...
public class SessionManager
{
            ...
            throws DTException 
    {
        ...
        RegisteredUser loginRegisteredUser = null;
        RegisteredUser knownRegisteredUser = null;
        Session s = null;
        ...
        loginRegisteredUser.setPassword( password );
        
        Iterator<RegisteredUser> persons = persistence.restoreRegisteredUser( loginRegisteredUser );
        if( persons.hasNext() ) {
             knownRegisteredUser = persons.next();
             System.out.println( "Have person!" );
             System.out.println( "id: " + knownRegisteredUser.getId() );
             System.out.println( "userName: " + knownRegisteredUser.getName() );
            ...
            //if(knownRegisteredUser.getPassword().equals(password))
     ...
     * @throws GVException
     */
    private static String createSession( Session session, RegisteredUser person ) 
            throws DTException
    {
        if( person == null ) {
        ...
        if( loggedIn.containsKey(person.getName()) ) {
            Session qs = loggedIn.get(person.getName());
            qs.setUser(person);
        ...
        loggedIn.put( person.getName(), session );
        session.start();
        return ssid;
    }
